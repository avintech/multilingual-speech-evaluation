...
def reading_task(file_content = None):
    try:
        lang = st.session_state.lang
        file_name = 'myfile2.wav'
    ...
    #Fluency
        if file_content == None:
            ...
            with open(file_name, mode='wb') as f:
                f.write(audio_bytes)
            audio_array, sampling_rate = librosa.load(file_name, sr=None)
            ...
            audio_wav.seek(0)
            audio_array, sampling_rate = librosa.load(audio_wav, sr=None)
...
def picture_task(file_content = None):
    try:
